<template>
  ...
  >
    ...
    <div class="flex flex-col gap-2">
      ...
      >
        <input
          v-model="tier.label"
          type="text"
          placeholder="Etiqueta (p. ej. Pack 2)"
        ...
        <input
          ...
          step="0.5"
          placeholder="€"
          class="w-24 rounded-lg border border-inaka-beige bg-white px-3 py-2 text-sm text-inaka-terra outline-none focus:border-inaka-terra"
        >
        <button
          type="button"
      ...
      <button
        type="button"
        class="self-start text-xs font-semibold text-inaka-gold hover:underline"
        @click="form.pricing.push({ label: '', price: 0 })"
      >
        + Añadir tramo
      </button>
    </div>
    ...
    <AdminChipListEditor
      v-model="form.sizes"
      label="Tamaños"
      placeholder="p. ej. 180cm"
    />
    <!-- Opciones/extras -->
    <AdminChipListEditor
      v-model="form.options"
      label="Opciones / extras"
      placeholder="p. ej. látex"
    />
    ...
    <div class="flex items-center gap-3">
      <button
        ...
        class="rounded-xl bg-inaka-terra px-6 py-3 text-sm font-semibold text-inaka-cream shadow-sm transition-opacity hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-40"
      >
