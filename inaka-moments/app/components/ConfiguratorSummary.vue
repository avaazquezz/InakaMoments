<template>
  <div class="flex flex-col">
    ...
    >
      <Icon
        name="lucide:shopping-cart"
        class="h-8 w-8 text-inaka-terra/30"
        aria-hidden="true"
      />
      <p class="text-sm text-inaka-terra/55">
        Aún no has añadido productos.
      </p>
      <p class="text-xs text-inaka-terra/40">
        Ve al paso «Productos» y crea tu combinación.
      </p>
    </div>
    ...
    <ul
      v-else
      class="flex flex-col divide-y divide-inaka-nude/70 px-5"
    >
      <li
      ...
      >
        <div class="min-w-0 flex-1">
          ...
          <p
            v-if="variantText(line)"
            class="mt-0.5 truncate text-xs text-inaka-terra/55"
          >
            {{ variantText(line) }}
          </p>
        ...
        <div class="shrink-0 text-right">
          <p class="text-sm font-bold text-inaka-terra whitespace-nowrap">
            <template v-if="view(line)?.consulta">
              A consultar
            </template>
            <template v-else>
              {{ formatEUR(view(line)!.line_total!) }}
            </template>
          </p>
          <p
            v-if="!view(line)?.consulta && line.qty > 1"
            class="text-[11px] text-inaka-terra/45 whitespace-nowrap"
          >
    ...
    <div
      v-if="count > 0"
      class="mt-1 flex flex-col gap-2 border-t border-inaka-nude px-5 py-4"
    >
      ...
      <p
        v-if="quote.hasConsulta"
        class="text-[11px] leading-snug text-inaka-terra/45"
      >
      ...
      <!-- Avisos -->
      <div
        v-if="quote.detallito"
        class="mt-1 flex items-center gap-2 rounded-lg bg-inaka-gold/12 px-3 py-2 text-xs font-medium text-inaka-terra"
      >
        <Icon
          name="lucide:gift"
          class="h-3.5 w-3.5 shrink-0"
          aria-hidden="true"
        /> ¡Tu pedido supera los {{ formatEUR(rules.umbral_detallito) }}! Incluye un detallito de regalo.
      </div>
      <div
        v-if="quote.hasRental"
        class="flex items-start gap-2 rounded-lg bg-inaka-mauve/12 px-3 py-2 text-xs leading-snug text-inaka-terra/75"
      >
        <Icon
          name="lucide:refresh-cw"
          class="mt-0.5 h-3.5 w-3.5 shrink-0"
          aria-hidden="true"
        /> Incluye alquiler de estructura: se abona una fianza reembolsable (a consultar).
      </div>
