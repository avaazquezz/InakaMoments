<template>
  <div class="flex flex-col gap-5">
    ...
    <AdminEmptyState
      ...
      message="Todos los eventos pasados ya tienen una solicitud de reseña enviada o programada."
    />
    ...
    >
      ...
      >
        <input
          v-model="selected"
          type="checkbox"
          class="mt-1 h-4 w-4 shrink-0 accent-inaka-terra"
          :value="c.quoteId"
        >
          ...
          <tbody class="divide-y divide-inaka-nude/70">
            ...
            >
              ...
              <td class="px-4 py-3 text-inaka-terra/60">
                {{ c.eventType ? EVENT_TYPE_LABELS[c.eventType] : '—' }}
              </td>
              <td class="px-4 py-3 text-inaka-terra/60">
                {{ c.eventDate }}
              </td>
...
async function sendSelected() {
  ...
  try {
    ...
    toast.error(apiErrorMessage(err, 'No se han podido enviar las solicitudes.'))
  }
  finally {
    sending.value = false
  }
