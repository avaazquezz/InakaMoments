<template>
  <div class="flex flex-col gap-6">
    ...
    <template v-else-if="data">
      <!-- Pendiente hoy -->
      <div>
        <h2 class="mb-3 text-sm font-bold text-inaka-terra">Pendiente hoy</h2>
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <NuxtLink to="/admin/leads" class="rounded-2xl bg-white p-5 ring-1 ring-inaka-nude transition-shadow hover:shadow-sm">
          ...
          <NuxtLink to="/admin/presupuestos?status=enviado" class="rounded-2xl bg-white p-5 ring-1 ring-inaka-nude transition-shadow hover:shadow-sm">
          ...
          <NuxtLink to="/admin/resenas" class="rounded-2xl bg-white p-5 ring-1 ring-inaka-nude transition-shadow hover:shadow-sm">
          ...
          <NuxtLink to="/admin/inventario" class="rounded-2xl bg-white p-5 ring-1 ring-inaka-nude transition-shadow hover:shadow-sm">
      ...
      <!-- Cómo va el negocio -->
      <div>
        <h2 class="mb-3 text-sm font-bold text-inaka-terra">Cómo va el negocio</h2>
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          <div class="rounded-2xl bg-white p-5 ring-1 ring-inaka-nude">
            <p class="text-xs font-semibold uppercase tracking-wide text-inaka-terra/60">Presupuestos aceptados</p>
            <p class="mt-2 text-3xl font-extrabold text-inaka-terra">{{ data.presupuestosAceptadosMes.count }}</p>
            <p class="mt-1 text-xs text-inaka-terra/70">{{ formatEUR(data.presupuestosAceptadosMes.total) }} en total, confirmados con fecha reservada</p>
            <p v-if="deltaPresupuestos" class="mt-1.5 text-xs font-semibold" :class="deltaPresupuestos.class">{{ deltaPresupuestos.text }}</p>
          </div>
          <div class="rounded-2xl bg-white p-5 ring-1 ring-inaka-nude">
            <p class="text-xs font-semibold uppercase tracking-wide text-inaka-terra/60">Reserva prevista</p>
            <p class="mt-2 text-3xl font-extrabold text-inaka-terra">{{ formatEUR(data.presupuestosAceptadosMes.senalPrevista) }}</p>
            <p class="mt-1 text-xs text-inaka-terra/70">% de reserva sobre el total, al aceptar</p>
          </div>
          <div class="rounded-2xl bg-white p-5 ring-1 ring-inaka-nude">
            <p class="text-xs font-semibold uppercase tracking-wide text-inaka-terra/60">Reserva cobrada</p>
            <p class="mt-2 text-3xl font-extrabold text-inaka-terra">{{ formatEUR(data.presupuestosAceptadosMes.senalCobrada) }}</p>
            <p class="mt-1 text-xs text-inaka-terra/70">Marcada "pagado" tras recibir el Bizum</p>
            <p v-if="deltaCobrada" class="mt-1.5 text-xs font-semibold" :class="deltaCobrada.class">{{ deltaCobrada.text }}</p>
          </div>
          <div class="rounded-2xl bg-white p-5 ring-1 ring-inaka-nude">
            <p class="text-xs font-semibold uppercase tracking-wide text-inaka-terra/60">Ticket medio</p>
            <p class="mt-2 text-3xl font-extrabold text-inaka-terra">{{ data.presupuestosAceptadosMes.ticketMedio != null ? formatEUR(data.presupuestosAceptadosMes.ticketMedio) : '—' }}</p>
            <p class="mt-1 text-xs text-inaka-terra/70">Valor medio de un presupuesto aceptado este mes</p>
          </div>
          <div class="rounded-2xl bg-white p-5 ring-1 ring-inaka-nude">
            <p class="text-xs font-semibold uppercase tracking-wide text-inaka-terra/60">Tasa de conversión</p>
            <p class="mt-2 text-3xl font-extrabold text-inaka-terra">{{ conversionPct }}%</p>
            <div class="mt-2 h-2 w-full overflow-hidden rounded-full bg-inaka-nude/50">
              <div class="h-full rounded-full bg-green-500 transition-all" :style="{ width: `${conversionPct}%` }" />
            </div>
            <p class="mt-1.5 text-xs text-inaka-terra/70">{{ conversionCaption }}</p>
          </div>
          <div class="rounded-2xl bg-white p-5 ring-1 ring-inaka-nude">
            <p class="text-xs font-semibold uppercase tracking-wide text-inaka-terra/60">Reseñas</p>
            <div class="mt-2 flex items-center gap-1.5">
              ...
              <svg v-if="data.resenas.mediaRating != null" class="h-5 w-5 text-inaka-gold" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true"><path d="M12 2l2.9 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l7.1-1.01L12 2z" /></svg>
            </div>
            <p class="mt-1 text-xs text-inaka-terra/70">{{ data.resenas.count }} publicada{{ data.resenas.count === 1 ? '' : 's' }}</p>
          </div>
        </div>
      </div>
      ...
      <div class="grid grid-cols-1 gap-4 lg:grid-cols-2">
        ...
        <div class="rounded-2xl bg-white p-5 ring-1 ring-inaka-nude">
          <h2 class="mb-3 text-sm font-bold text-inaka-terra">
            Próximos eventos
            <span v-if="data.eventosProximos7Dias > 0" class="ml-1 font-normal text-inaka-terra/70">· {{ data.eventosProximos7Dias }} en los próximos 7 días</span>
          </h2>
          <AdminEmptyState v-if="data.proximosEventos.length === 0" title="Sin eventos próximos" message="Los eventos confirmados o sin confirmar aparecerán aquí." />
          <ul v-else class="flex flex-col divide-y divide-inaka-nude/70">
            <li v-for="ev in data.proximosEventos" :key="ev.id" class="flex items-center justify-between gap-3 py-2.5">
              <div class="min-w-0">
                ...
                <p class="text-xs text-inaka-terra/70">{{ formatFecha(ev.event_date) }}</p>
              </div>
      ...
      <!-- Embudo de leads -->
      <div class="rounded-2xl bg-white p-5 ring-1 ring-inaka-nude">
        <h2 class="mb-3 text-sm font-bold text-inaka-terra">Estado de los clientes</h2>
        <div class="flex flex-col gap-2.5">
          <div v-for="f in data.funnel" :key="f.status" class="flex items-center gap-3">
            ...
            <div class="h-2.5 flex-1 overflow-hidden rounded-full bg-inaka-nude/50">
              <div class="h-full rounded-full" :style="{ width: `${maxFunnel ? (f.count / maxFunnel) * 100 : 0}%`, backgroundColor: funnelColor(f.status) }" />
            </div>
        ...
        <div class="mt-4 flex flex-wrap items-center gap-x-4 gap-y-2 border-t border-inaka-beige/70 pt-3">
          <div v-for="s in LEAD_STATUSES" :key="s" class="flex items-center gap-1.5">
            <span class="h-2.5 w-2.5 shrink-0 rounded-full" :style="{ backgroundColor: LEAD_STATUS_STYLES[s].bar }" />
