<template>
  <div class="flex flex-col gap-5">
    ...
    <div v-else class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-5">
      <div v-for="col in columns" :key="col.status" class="flex flex-col gap-2">
        <p class="px-1 text-xs font-bold uppercase tracking-wide text-inaka-terra/50">{{ col.label }} ({{ leadsByStatus(col.status).length }})</p>
        <div class="flex flex-col gap-2">
          <div v-for="lead in leadsByStatus(col.status)" :key="lead.id" class="rounded-xl bg-white p-3 ring-1 ring-inaka-nude">
            <NuxtLink :to="`/admin/leads/${lead.id}`" class="block">
              ...
              <p v-if="lead.tipo" class="mt-1 text-xs text-inaka-terra/40">{{ lead.tipo }}</p>
            </NuxtLink>
            <select
          ...
          <p v-if="leadsByStatus(col.status).length === 0" class="rounded-xl border border-dashed border-inaka-beige px-3 py-6 text-center text-xs text-inaka-terra/30">Vacío</p>
        </div>
      </div>
    </div>
...
async function changeStatus(lead: AdminLead, status: string) {
  try {
    ...
    toast.error(err?.data?.message ?? 'No se ha podido cambiar el estado.')
  }
}
...
async function createLead() {
  ...
  try {
    ...
    toast.error(err?.data?.message ?? 'No se ha podido crear el lead.')
  }
  finally {
