<template>
  <div class="flex flex-col gap-5">
    <div class="flex items-center justify-end">
      <button type="button" class="inline-flex items-center gap-1.5 rounded-xl bg-inaka-terra px-4 py-2.5 text-sm font-semibold text-inaka-cream hover:opacity-90" @click="creating = true">
    ...
    <AdminEmptyState v-else-if="(data ?? []).length === 0" title="Sin álbumes todavía" message="Crea el primero para empezar a subir fotos." />

    <div v-else class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
      <NuxtLink v-for="a in data" :key="a.id" :to="`/admin/galeria/${a.id}`" class="flex flex-col gap-2 rounded-2xl bg-white p-4 ring-1 ring-inaka-nude transition-shadow hover:shadow-sm">
        <div class="flex items-start justify-between gap-2">
          <p class="font-bold text-inaka-terra">{{ a.title }}</p>
          <span class="rounded-full px-2 py-0.5 text-[11px] font-semibold" :class="a.published ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'">
        ...
        <div class="relative max-h-[90vh] w-full max-w-sm overflow-y-auto rounded-2xl bg-white p-6 shadow-xl">
          ...
          <form class="flex flex-col gap-4" @submit.prevent="createAlbum">
            <div class="flex flex-col gap-1.5">
              ...
              <input v-model="form.title" type="text" required class="rounded-lg border border-inaka-beige bg-white px-3 py-2 text-sm text-inaka-terra outline-none focus:border-inaka-terra" />
            </div>
            <div class="flex flex-col gap-1.5">
...
async function createAlbum() {
  ...
  try {
    ...
    toast.error(err?.data?.message ?? 'No se ha podido crear el álbum.')
  }
  finally {
