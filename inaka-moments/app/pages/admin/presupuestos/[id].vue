    ...
    <template v-else-if="quote">
      ...
      <div class="flex flex-wrap items-center justify-between gap-3 rounded-2xl bg-white p-5 ring-1 ring-inaka-nude">
        <div class="flex items-center gap-3">
          ...
          <span
            v-if="quote.event_type"
            class="text-sm text-inaka-terra/50"
          >{{ EVENT_TYPE_LABELS[quote.event_type] }}</span>
          <AdminStatusBadge
            :status="quote.status"
            kind="quote"
          />
        ...
        <div class="flex flex-wrap items-center gap-2">
          ...
          <button
            v-if="quote.status !== 'aceptado'"
            type="button"
            class="rounded-lg px-3 py-1.5 text-xs font-semibold text-red-500 hover:underline"
      ...
      <div class="grid grid-cols-1 gap-5 lg:grid-cols-3">
        ...
        <form
          class="flex flex-col gap-4 rounded-2xl bg-white p-5 ring-1 ring-inaka-nude lg:col-span-2"
        ...
        >
          ...
          <button
            type="submit"
            :disabled="savingInfo"
            class="self-start rounded-xl bg-inaka-terra px-5 py-2.5 text-sm font-semibold text-inaka-cream hover:opacity-90"
          >
        ...
        <div class="rounded-2xl bg-white p-5 ring-1 ring-inaka-nude">
          <h3 class="mb-3 text-sm font-bold text-inaka-terra">
            Resumen
          </h3>
          <div class="flex items-center justify-between text-sm">
            <span class="text-inaka-terra/60">Subtotal</span><span class="font-semibold text-inaka-terra">{{ formatEUR(quote.subtotal) }}</span>
          </div>
          <div
          ...
          <div class="mt-2 flex items-center justify-between gap-2">
            <p class="text-xs text-inaka-terra/50">
              <template v-if="quote.status === 'aceptado'">
                Reserva: {{ formatEUR(quote.deposit_amount!) }}
              </template>
              <template v-else>
                Reserva prevista ({{ senalPorcentaje }}%): {{ formatEUR(round2(quote.total * senalPorcentaje / 100)) }}
              </template>
            </p>
            <template v-if="quote.status === 'aceptado'">
              <AdminStatusBadge
                :status="quote.deposit_status"
                kind="payment"
              />
              <select
                v-model="quote.deposit_status"
                class="rounded-lg border border-inaka-beige bg-white px-2 py-1 text-xs text-inaka-terra outline-none focus:border-inaka-terra"
                @change="updateDepositStatus"
              >
                <option
      ...
      <div class="rounded-2xl bg-white p-5 ring-1 ring-inaka-nude">
        ...
        >
          <div
            v-for="(line, i) in draftLines"
            :key="i"
            class="flex items-center gap-2 rounded-lg border border-inaka-beige p-2"
          >
            ...
            <input
              ...
              type="number"
              min="1"
              class="w-16 rounded border border-inaka-beige px-2 py-1 text-sm text-inaka-terra outline-none focus:border-inaka-terra"
            >
            <button
              type="button"
              class="text-red-500"
          ...
          <div class="flex flex-wrap items-center gap-2 rounded-lg bg-inaka-cream p-3">
            <select
              ...
              class="rounded-lg border border-inaka-beige bg-white px-2 py-1.5 text-sm text-inaka-terra outline-none focus:border-inaka-terra"
            >
              <option value="">
                Añadir producto…
              </option>
            ...
            <input
              ...
              type="number"
              min="1"
              class="w-16 rounded-lg border border-inaka-beige bg-white px-2 py-1.5 text-sm text-inaka-terra outline-none focus:border-inaka-terra"
            >
            <button
              type="button"
              class="rounded-lg bg-inaka-terra px-3 py-1.5 text-xs font-semibold text-inaka-cream hover:opacity-90"
          ...
          <button
            type="button"
            :disabled="savingItems || draftLines.length === 0"
            class="self-start rounded-xl bg-inaka-terra px-5 py-2.5 text-sm font-semibold text-inaka-cream hover:opacity-90"
            @click="saveItems"
          >
      ...
      <div class="rounded-2xl bg-white p-5 ring-1 ring-inaka-nude">
        <h3 class="mb-3 text-sm font-bold text-inaka-terra">
          Ajustes manuales (descuentos, extras…)
        </h3>
        <div
          v-for="(adj, i) in manualAdjustments"
          :key="i"
        ...
        >
          ...
          <input
            ...
            type="number"
            step="0.5"
            placeholder="€ (negativo = descuento)"
            class="w-40 rounded-lg border border-inaka-beige bg-white px-3 py-2 text-sm text-inaka-terra outline-none focus:border-inaka-terra"
          >
          <button
            type="button"
            class="text-red-500"
        ...
        <div class="flex items-center gap-3">
          <button
            ...
            @click="manualAdjustments.push({ key: `manual-${Date.now()}`, label: '', amount: 0 })"
          >
            + Añadir ajuste
          </button>
          <button
            ...
            class="rounded-lg bg-inaka-terra px-4 py-2 text-xs font-semibold text-inaka-cream hover:opacity-90"
...
async function saveInfo() {
  ...
  catch (err) { toast.error(apiErrorMessage(err, 'No se ha podido guardar.')) }
...
async function saveItems() {
  ...
  catch (err) { toast.error(apiErrorMessage(err, 'No se ha podido recalcular.')) }
...
async function confirmAccept() {
  ...
  try {
    ...
    acceptError.value = apiErrorMessage(err, 'No se ha podido aceptar el presupuesto.')
  }
  finally {
...
async function updateDepositStatus() {
  ...
  try {
    ...
    toast.error(apiErrorMessage(err, 'No se ha podido actualizar la reserva.'))
    await refresh()
  }
}
...
async function deleteQuote() {
  ...
  catch (err) { toast.error(apiErrorMessage(err, 'No se ha podido borrar.')) }
