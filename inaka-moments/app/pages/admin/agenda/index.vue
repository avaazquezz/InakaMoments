    ...
    <div v-else class="overflow-hidden rounded-2xl bg-white ring-1 ring-inaka-nude">
      ...
      <table v-else class="w-full text-left text-sm">
        ...
        <tbody class="divide-y divide-inaka-nude/70">
          <tr v-for="ev in sortedEvents" :key="ev.id" class="cursor-pointer hover:bg-inaka-cream/50" @click="openEdit(ev)">
            <td class="px-4 py-3 font-medium text-inaka-terra">{{ ev.title }}</td>
            <td class="px-4 py-3 text-inaka-terra/60">{{ ev.event_date }}</td>
            <td class="px-4 py-3"><AdminStatusBadge :status="ev.status" kind="event" /></td>
          </tr>
        </tbody>
      </table>
    </div>
...
async function save() {
  ...
  try {
    ...
    saveError.value = err?.data?.message ?? 'No se ha podido guardar el evento.'
  }
  finally {
    saving.value = false
  }
