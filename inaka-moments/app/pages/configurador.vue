  ...
  <main class="bg-inaka-cream">
    ...
    <section class="relative overflow-hidden border-b border-inaka-nude bg-white py-12 sm:py-16">
      ...
      <div class="relative mx-auto max-w-6xl px-4 text-center sm:px-6 lg:px-8">
        <p class="mb-3 text-xs font-semibold uppercase tracking-widest text-inaka-gold">
          Tú eliges, nosotros creamos
        </p>
        <h1 class="font-display text-3xl font-bold text-inaka-terra sm:text-4xl lg:text-5xl">
          ...
          >
            ...
            >
              ...
              <span
                ...
                :class="s.n === step ? 'text-inaka-terra' : 'text-inaka-terra/50'"
              >{{ s.label }}</span>
            </button>
            <span
              v-if="s.n < steps.length"
              class="h-px flex-1 bg-inaka-nude"
              aria-hidden="true"
            />
          </li>
        </ol>
      ...
      <div class="rounded-3xl bg-white p-8 text-center shadow-sm ring-1 ring-inaka-nude sm:p-12">
        <div class="mx-auto mb-5 flex h-20 w-20 items-center justify-center rounded-full bg-inaka-gold/15">
          <Icon
            ...
            class="h-10 w-10 text-inaka-gold"
            aria-hidden="true"
          />
        </div>
        <h2 class="font-display text-2xl font-bold text-inaka-terra sm:text-3xl">
          ¡Propuesta enviada!
        </h2>
        ...
        <div
          v-if="result"
          class="mx-auto mt-8 max-w-md rounded-2xl bg-inaka-cream p-5 text-left ring-1 ring-inaka-nude"
        >
          <p class="mb-3 text-xs font-semibold uppercase tracking-widest text-inaka-gold">
          ...
          <ul class="flex flex-col divide-y divide-inaka-nude/70">
            ...
            <li
              v-for="adj in result.adjustments"
              :key="adj.key"
              class="flex items-center justify-between gap-3 py-2 text-sm"
            >
          ...
          <div class="mt-3 flex items-center justify-between border-t border-inaka-nude pt-3">
            ...
            <span class="text-xl font-extrabold text-inaka-terra">{{ formatEUR(result.total) }}<span
              v-if="result.hasConsulta"
        ...
        <div class="mt-8 flex flex-col justify-center gap-3 sm:flex-row">
          ...
          <NuxtLink
            to="/catalogo"
            class="rounded-xl border border-inaka-terra px-6 py-3 text-sm font-semibold text-inaka-terra outline-none transition-colors hover:bg-inaka-nude focus-visible:ring-2 focus-visible:ring-inaka-gold"
          >
            Volver al catálogo
          </NuxtLink>
        </div>
    ...
    <section
      v-else
      class="mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:px-8 lg:py-12"
    >
      <div class="grid grid-cols-1 gap-8 lg:grid-cols-3">
        ...
        <div class="lg:col-span-2">
          ...
          <div
            ...
            key="s1"
          >
            <h2 class="font-display text-2xl font-bold text-inaka-terra">
              ¿Qué celebras?
            </h2>
            <p class="mb-6 mt-1 text-sm text-inaka-terra/55">
              Elige la ocasión para personalizar tu presupuesto.
            </p>
            <div class="grid grid-cols-2 gap-3 sm:grid-cols-3">
              ...
              >
                <Icon
                  :name="opt.icon"
                  class="h-8 w-8 text-inaka-gold transition-transform duration-200 group-hover:scale-110"
                  aria-hidden="true"
                />
          ...
          >
            ...
            <p class="mb-6 mt-1 text-sm text-inaka-terra/55">
              La fecha nos ayuda a comprobar disponibilidad. Puedes ajustarla más adelante.
            </p>
            ...
            <div class="flex max-w-md flex-col gap-5">
              <div class="flex flex-col gap-1.5">
                ...
                >Fecha aproximada del evento</label>
                <DatePicker
                  id="cfg-fecha"
              ...
              <div class="flex flex-col gap-1.5">
                ...
                <p
                  ...
                  class="flex items-center gap-1.5 text-xs text-inaka-terra/50"
                >
                  <Icon
                ...
                <p
                  v-else-if="geocodeStatus === 'ok' && state.distance_km != null"
                  class="flex items-center gap-1.5 text-xs"
                  :class="isFarAway ? 'text-inaka-terra/70' : 'text-inaka-terra/50'"
                >
                  <Icon
                    name="lucide:map-pin"
                    class="h-3 w-3 shrink-0"
                ...
                <label
                  v-if="geocodeStatus === 'idle' || geocodeStatus === 'error'"
                  class="mt-1 flex cursor-pointer items-start gap-3 rounded-xl border border-inaka-beige bg-white p-4 transition-colors hover:border-inaka-terra/40"
                >
                  <input
                    v-model="state.far"
                    type="checkbox"
              ...
              <div class="flex flex-col gap-1.5">
                ...
                <div class="relative">
                  ...
                  >
                    ...
                    <option value="Más de 100">
                      Más de 100 personas
                    </option>
                  </select>
                  <Icon
                    name="lucide:chevron-down"
                    class="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 h-4 w-4 text-inaka-terra/40"
                    aria-hidden="true"
                  />
                </div>
              </div>
          ...
          >
            ...
            <p class="mb-5 mt-1 text-sm text-inaka-terra/55">
              Añade lo que quieras y ajusta cantidades. Todo es combinable entre sí.
            </p>

            <div
              v-if="productsPending"
              class="py-16 text-center text-inaka-terra/50"
            >
              Cargando catálogo…
            </div>
            ...
            <template v-else>
                    ...
                    <div class="flex items-start justify-between gap-2">
                      <div>
                        ...
                        <p class="text-sm font-semibold text-inaka-terra/70">
                          {{ pk.price != null ? formatEUR(pk.price) : 'A consultar' }}
                        </p>
                      </div>
                      <span
                        v-if="countOf('pack', pk.id)"
                        class="rounded-full bg-inaka-terra px-2 py-0.5 text-xs font-bold text-inaka-cream"
                      >× {{ countOf('pack', pk.id) }}</span>
                    </div>
                    <p class="line-clamp-2 text-xs text-inaka-terra/55">
                      {{ pk.description }}
                    </p>
                    <button
                      type="button"
                ...
                >
                  <div class="flex items-start gap-3">
                    <div class="flex h-14 w-14 shrink-0 items-center justify-center overflow-hidden rounded-xl bg-inaka-nude/40">
                      ...
                      <span
                        ...
                        aria-hidden="true"
                      >{{ categoryEmoji(p.category) }}</span>
                    </div>
                    <div class="min-w-0 flex-1">
                      <p class="text-sm font-bold leading-snug text-inaka-terra">
                        {{ p.name }}
                      </p>
                      <p class="text-sm font-semibold text-inaka-terra/70">
                        {{ productPriceLabel(p) }}
                      ...
                      <span
                        v-if="countOf('product', p.id)"
                        class="mt-1 inline-block rounded-full bg-inaka-gold/20 px-2 py-0.5 text-[11px] font-bold text-inaka-terra"
                      >En tu presupuesto: {{ countOf('product', p.id) }}</span>
                    </div>
                  </div>
                  ...
                  <div class="mt-auto flex items-center gap-2">
                    <div class="flex items-center rounded-lg border border-inaka-beige">
                      <button
                        type="button"
                        class="flex h-8 w-8 items-center justify-center text-inaka-terra/70 outline-none hover:bg-inaka-nude/50 focus-visible:ring-2 focus-visible:ring-inaka-gold"
                        :aria-label="`Menos ${p.name}`"
                        @click="draftFor(p).qty = Math.max(1, draftFor(p).qty - 1)"
                      >
                        <Icon
                          name="lucide:minus"
                          class="h-3.5 w-3.5"
                      ...
                      <button
                        ...
                        @click="draftFor(p).qty = Math.min(99, draftFor(p).qty + 1)"
                      >
                        <Icon
                          name="lucide:plus"
                          class="h-3.5 w-3.5"
                    ...
                    >
                      <Icon
                        :name="justAdded === p.id ? 'lucide:check' : 'lucide:plus'"
                        class="h-3.5 w-3.5"
                        aria-hidden="true"
                      />
              ...
              <p
                ...
                class="py-10 text-center text-sm text-inaka-terra/50"
              >
          ...
          >
            ...
            <p class="mb-6 mt-1 text-sm text-inaka-terra/55">
              Te enviaremos la propuesta y te contactaremos para confirmar. Campos con <span class="font-bold text-inaka-mauve">*</span> obligatorios.
            </p>
            ...
            <div class="flex max-w-md flex-col gap-5">
              ...
              <div class="flex flex-col gap-1.5">
                <label
                  for="cfg-nombre"
                  class="text-sm font-semibold text-inaka-terra"
                >Nombre completo <span class="text-inaka-mauve">*</span></label>
                <input
                  id="cfg-nombre"
                ...
                >
                <p
                  v-if="touched.nombre && !contact.nombre.trim()"
              ...
              <div class="flex flex-col gap-1.5">
                <label
                  for="cfg-email"
                  class="text-sm font-semibold text-inaka-terra"
                >Correo electrónico <span class="text-inaka-mauve">*</span></label>
                <input
                  id="cfg-email"
                ...
                >
                <p
                  v-if="touched.email && !emailValido"
                  class="text-xs text-red-500"
                >
              ...
              <div class="flex flex-col gap-1.5">
                <label
                  for="cfg-tel"
                  class="text-sm font-semibold text-inaka-terra"
                >WhatsApp / Teléfono <span class="text-inaka-mauve">*</span></label>
                <div
                  class="flex overflow-hidden rounded-xl border transition-all"
                ...
                >
                  <div class="relative shrink-0">
                    <select
                      ...
                      class="h-full cursor-pointer appearance-none border-r border-inaka-beige bg-inaka-nude/60 pl-3 pr-7 text-sm font-medium text-inaka-terra outline-none focus-visible:ring-2 focus-visible:ring-inaka-gold"
                      aria-label="Prefijo de país"
                    >
                      <option
                        v-for="pfx in prefijos"
                        :key="pfx.code"
                        :value="pfx.dial"
                      >
                        {{ pfx.flag }} {{ pfx.dial }}
                    ...
                    <Icon
                      name="lucide:chevron-down"
                      class="pointer-events-none absolute right-1.5 top-1/2 h-3 w-3 -translate-y-1/2 text-inaka-terra/50"
                      aria-hidden="true"
                    />
                  </div>
                  <input
                    ...
                    class="flex-1 bg-white px-4 py-3 text-sm text-inaka-terra placeholder:text-inaka-terra/30 outline-none focus-visible:ring-2 focus-visible:ring-inaka-gold"
                    @blur="touched.telefono = true"
                  >
                </div>
                <p
                  v-if="touched.telefono && !telefonoValido"
                  class="text-xs text-red-500"
                >
              ...
              <div class="flex flex-col gap-1.5">
                ...
                <textarea
                  ...
                  rows="3"
                  placeholder="Colores, temática, alguna idea concreta…"
              ...
              <div
                v-if="submitError"
                class="flex gap-3 rounded-xl border border-red-200 bg-red-50 px-4 py-3"
              >
                <Icon
          ...
          <div class="mt-8 flex items-center justify-between gap-3">
            ...
            >
              <Icon
                :name="isSending ? 'lucide:loader-2' : 'lucide:send'"
                class="h-4 w-4"
    ...
    <div
      v-if="!done"
      class="fixed inset-x-0 bottom-0 z-40 border-t border-inaka-nude bg-white/95 p-3 shadow-[0_-4px_20px_rgba(0,0,0,0.06)] backdrop-blur-sm lg:hidden"
    >
      <button
        type="button"
    ...
    <Transition name="sheet">
      ...
      >
        <div
          class="absolute inset-0 bg-inaka-terra/30 backdrop-blur-sm"
        ...
        <div class="absolute inset-x-0 bottom-0 max-h-[85vh] overflow-y-auto rounded-t-3xl bg-white">
          <div class="sticky top-0 flex items-center justify-between border-b border-inaka-nude bg-white px-5 py-3">
            ...
            <button
              type="button"
              class="flex h-8 w-8 items-center justify-center rounded-full text-inaka-terra/60 outline-none hover:bg-inaka-nude focus-visible:ring-2 focus-visible:ring-inaka-gold"
              aria-label="Cerrar"
              @click="sheetOpen = false"
            >
              <Icon
...
function selectOccasion(value: string) {
  ...
  step.value = 2
  track('configurador_paso', { paso: 2, ocasion: value })
}
...
function goNext() {
  ...
  if (step.value < 4) {
    step.value++
    track('configurador_paso', { paso: step.value })
  }
}
...
async function submit() {
  ...
  try {
    ...
    track('presupuesto_enviado', { ocasion: state.value.event_type, total: res.quote?.total })
    done.value = true
